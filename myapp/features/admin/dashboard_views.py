...
from ...models import User, UserProfile, Patient, LabResult, Appointment, BookedService, RolePermission, Prescription
...
def moddashboard(request):
    ...
    if request.session.get("is_admin"):
        ...
        latest_prescriptions = Prescription.objects.select_related('live_appointment__appointment__patient', 'doctor__user').order_by('-created_at')[:5]
...
def get_password_reset_notifications(request):
    """Get all password reset and patient message notifications for admin"""
    try:
        from ...models import Notification
        from django.db.models import Q
        
        # Get both password reset and system notifications (patient messages)
        notifications = Notification.objects.filter(
            Q(notification_type='password_reset') | Q(notification_type='system')
        ).order_by('-created_at')[:50]
        
        notif_data = []
        for notif in notifications:
            # Get the related user ID (the person who made the request)
            related_user_id = notif.related_id if notif.related_id else None
            
            notif_data.append({
                'notification_id': notif.notification_id,
                'title': notif.title or 'Notification',
                'message': notif.message or '',
                'user_id': notif.user.user_id if notif.user else None,  # Admin who receives it
                'related_id': related_user_id,  # User who made the request
        ...
        # Count unread password reset notifications
        ...
        import traceback
        print(f"Error in get_password_reset_notifications: {traceback.format_exc()}")
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)
...
def batch_delete_notifications(request):
    """Delete oldest notifications in batch (10, 30, or 50) for current admin user only"""
    try:
        ...
        # Validate count is one of the allowed values
        if count not in [10, 30, 50]:
            return JsonResponse({
                'success': False,
                'error': 'Invalid count. Must be 10, 30, or 50'
            }, status=400)
        
        # Delete oldest notifications from ALL notifications in the entire table
        # No filtering by user or type - just get the oldest X notifications
        oldest_notifications = Notification.objects.all().order_by('created_at').values_list('notification_id', flat=True)[:count]
        
        # Convert to list to evaluate the queryset
        notification_ids = list(oldest_notifications)
        deleted_count = len(notification_ids)
        
        # Delete ONLY these specific notification IDs
        if notification_ids:
            Notification.objects.filter(notification_id__in=notification_ids).delete()
        else:
            deleted_count = 0
        ...
        import traceback
        print(f"Error in batch_delete_notifications: {traceback.format_exc()}")
        return JsonResponse({
            'success': False,
            'error': str(e)
        }, status=500)
