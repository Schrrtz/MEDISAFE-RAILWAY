...
class UserProfile(models.Model):
    ...
    photo_url = models.TextField(null=True, blank=True)  # Stores base64 data URL (data:image/jpeg;base64,...)
...
class Notification(models.Model):
    ...
    file = models.TextField(null=True, blank=True)  # Stores base64 data URL for attachments
...
class LiveAppointment(models.Model):
    ...
    # Stores the appointment_number from the linked appointment for display consistency.
    # This ensures the same appointment number is shown across all interfaces.
    live_appointment_number = models.CharField(max_length=20, unique=True, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        # Store appointment_number from the linked appointment instead of generating LAP### code
        if not self.live_appointment_number and self.appointment:
            self.live_appointment_number = self.appointment.appointment_number
...
class Prescription(models.Model):
    ...
    # Prescription file (PDF, image, etc. uploaded by doctor) - stored as base64 data URL
    prescription_file = models.TextField(null=True, blank=True)
...
class WatchVitals(models.Model):
    """Model for smartwatch vital signs data"""
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='watch_vitals',
        db_column='user_id',
        to_field='user_id'
    )
    device_id = models.CharField(max_length=100, null=True, blank=True)
    heart_rate = models.IntegerField(null=True, blank=True)  # bpm
    systolic = models.IntegerField(null=True, blank=True)  # mmHg
    diastolic = models.IntegerField(null=True, blank=True)  # mmHg
    ...
    captured_at = models.DateTimeField(null=True, blank=True)  # When the watch captured the data
    created_at = models.DateTimeField(null=True, blank=True)  # When record was created in DB
    measured_by = models.CharField(max_length=255, null=True, blank=True)
    ...
    def __str__(self):
        return f"Vitals for {self.user.username} - {self.captured_at.strftime('%Y-%m-%d %H:%M:%S') if self.captured_at else 'No timestamp'}"
